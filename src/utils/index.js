...
const headers = [
  "I'm going to roll the stats for a",
  "Check it, I'm gonna be a",
  'Since I was little, I have always wanted to be a',
  'When else would I be able to be a',
];

const buttonTextOptions = [
  "That doesn't sound interesting",
  'No thanks on that one',
  "This doesn't speak to my soul",
  'That sounds boring',
  "Give me something that's more rad than that",
  'I really need something that I can connect with on a spiritual level',
  'Nopity no',
  'I want something more awesome',
];
...
const generateBodyText = () => {
  const adj = getRandomArrayElement(adjectives);
  const race = getRandomArrayElement(races);
  const dclass = getRandomArrayElement(dclasses);
  const loc = getRandomArrayElement(locations);
  const back = getRandomArrayElement(backstories);

  return `${adj} ${race} ${dclass} from ${loc} who ${back}`;
};
